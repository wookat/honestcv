// R825: replay the 186 retained texts against the frozen R824 (HEAD 6e0dfcc) parser (/home/ubuntu/qa/r825-wt)
// and print every difference.
// Run: npx vite-node qa/r825-replay.mts
import { readFileSync } from 'node:fs'
import { parseResumeText } from '../src/lib/importText'
import { parseResumeText as parseOld } from '../../../qa/r825-wt/src/lib/importText'
...
for (const k of Object.keys(texts).sort()) {
  ...
  console.log(`  title ${JSON.stringify(a.contact.title)} → ${JSON.stringify(b.contact.title)}; location ${JSON.stringify(a.contact.location)} → ${JSON.stringify(b.contact.location)}; name ${JSON.stringify(a.contact.fullName)} → ${JSON.stringify(b.contact.fullName)}`)
