...
| **P1** | Tailor suggestions ignore the job's missing keywords (60 % → 60 % on the R712 sample) | `qa/r712-tailor-1.json`: 5 rewrites, none mentions `graphql` / `next.js` | **R715**: pass `missingKeywords` into the tailor prompt and post-check; surface "covers: graphql" per line; keep R712 grounding flags |
