...
| **P1** | Interview prep is a static brief + Q list with one-shot feedback, not a practice loop | Rezi AI Interview: ask → answer → STAR feedback → score, per question | **R707** (if budget): question-by-question practice mode reusing `/api/ai/interview-questions` + `/api/ai/interview-feedback` with a running score. |
