...
| **P1 (resource, degraded R716)** | Job search coverage / location filter | `qa/r713-prod.json`: nurse 3 results, London query 2/5 London; `qa/r716-*.json` (948 prod postings): a "London" filter hid 30 `UK` / `United Kingdom` / `EMEA, UK` / `Europe` postings, "UK" matched `Ukraine` | **R716** (docs/plan-r716-location-tiers.md): results split direct → "Open to UK / Europe / EMEA (N)" → "Open to any location (N)" from a feed-label gazetteer (63+12 → 63+30+12 shown for London). Non-remote local coverage still needs Adzuna/JSearch keys |
