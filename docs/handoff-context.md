...
- R24 操作台导入磁贴 + 相对时间戳（2026-08-29 登录复审计 ~/audit-r1/shots-r24/：Rezi resumes 页建简历卡支持「or drop a resume here」且卡片显示「Edited 3 hours ago」；/dashboard My resumes 网格新增虚线「Import a resume」磁贴——点选/拖放 PDF/DOCX/TXT 浏览器内 extractTextFromFile+parseResumeText 解析后写入草稿跳 /builder，已有草稿时弹确认弹窗（可先存副本），无文本/读取失败磁贴内联报错；版本卡片日期改「Edited today/N days ago」相对时间；纯前端无存储/Worker 变更）
...
- PR 为累积链（R2 基于 R1 分支…R24 基于 R23），生产已部署最新版（R21 部署时 wrangler 上传成功但 zones/routes 同步 API 报 Authentication error 10000——自定义域路由已存在故不影响上线，新 bundle 已实测生效；若后续需改路由需补 Cloudflare token 的 zone Workers Routes 权限）；PR 均未合并，合并时按链顺序
