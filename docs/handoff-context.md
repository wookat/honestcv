...
- R23 操作台文档类型筛选（对标 Rezi 侧栏 RESUMES/COVER LETTERS/RESIGNATION LETTERS 分列：/dashboard「Career documents」标题下新增类型筛选 pill——All/Cover letters/Interview prep/Resignation letters 带计数，零计数类型不显示，删除某类型最后一份自动回落 All；纯前端客户端过滤，honestcv.careerDocs 存储不变）
...
- PR 为累积链（R2 基于 R1 分支…R23 基于 R22），生产已部署最新版（R21 部署时 wrangler 上传成功但 zones/routes 同步 API 报 Authentication error 10000——自定义域路由已存在故不影响上线，新 bundle 已实测生效；若后续需改路由需补 Cloudflare token 的 zone Workers Routes 权限）；PR 均未合并，合并时按链顺序
