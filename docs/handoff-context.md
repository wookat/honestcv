...
- R26 面试答案练习+AI 反馈（#TBD，2026-08-29 登录复审计 ~/audit-r1/shots-r26/：Rezi「AI INTERVIEW」为视频面试模拟器、「AI RESUME AGENT」为聊天；诚实降级为文本练习——Interview Prep 弹窗底部新增「Practice an answer」区：问题+答案输入，Worker 新端点 POST /api/ai/interview-feedback（与 interview-brief 同 bundle/免费额度门控，输出 WHAT WORKED/WHAT TO IMPROVE/STRONGER ANSWER，占位符不造经历）；Agent 聊天面属更大架构决策留待后续轮）
