    ...
    <h3
      ...
      style={{
        ...
        borderBottom:
          divider === "none"
            ? "none"
            : `${divider === "thick" ? 2 : 1}px solid ${tpl.accent}`,
        paddingBottom: tpl.band ? 3 : divider === "none" ? 0 : 3,
        ...(tpl.band
          ? {
              background: accentTint(tpl.accent),
              padding: "3px 6px",
              borderRadius: 2,
            }
          : {}),
          ...
          >
            <div style={{ height: windowH, overflow: "hidden" }}>
              <div
                ...
                style={{
                  transform:
                    i > 0 ? `translateY(-${i * windowH}px)` : undefined,
                }}
              >
...
function SectionBlock({
          ...
          <div key={c.id} className="mb-1.5">
            <div className="flex flex-wrap items-baseline justify-between gap-x-2">
              <p className="text-[11px] font-bold">
                ...
                {c.issuer.trim() && (
                  <span className="font-normal"> — {c.issuer.trim()}</span>
                )}
              </p>
              {c.date.trim() && (
                <p className="text-[10px] text-neutral-500 italic">
  ...
  if (sectionKey.startsWith("custom:")) {
    const s = resume.customSections.find(
      (x) => `custom:${x.id}` === sectionKey,
    );
    if (!s || (!s.title.trim() && !s.bullets.some((b) => b.trim())))
      return null;
    return (
      <>
