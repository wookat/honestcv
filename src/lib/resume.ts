...
export interface Resume {
  ...
  experienceLevel?:
    "" | "internship" | "entry" | "mid" | "senior" | "executive";
...
export const lineSpacingOf = (r: Resume) =>
  LINE_SPACING[r.lineSpacing ?? "normal"];

export type FontFamilyKind =
  "serif" | "sans" | "mono" | "merriweather" | "sourcesans" | "robotomono";
...
export const sectionSpacingOf = (r: Resume) =>
  SECTION_SPACING[r.sectionSpacing ?? "normal"];
...
export function sortEntriesByDate<T>(
  ...
  const keyed = items.map((item, index) => {
    ...
    const end = ONGOING_RE.test(endOf(item))
      ? Number.MAX_SAFE_INTEGER
      : dateSortValue(endOf(item));
    const primary = end ?? start;
    return {
      item,
      index,
      primary,
      start: start ?? end ?? Number.MIN_SAFE_INTEGER,
    };
  });
  keyed.sort((a, b) => {
...
export function sanitizeResume(input: unknown): Resume | null {
  ...
  const resume: Resume = {
    ...
    lineSpacing: asEnum(raw.lineSpacing, [
      "xtight",
      "compact",
      "normal",
      "relaxed",
    ...
    fontFamily: asEnum(raw.fontFamily, [
      "auto",
      "serif",
      "sans",
      "mono",
    ...
    sectionSpacing: asEnum(raw.sectionSpacing, [
      "xtight",
      "tight",
      "normal",
      "roomy",
    ...
    experienceLevel: asEnum(raw.experienceLevel, [
      "internship",
      "entry",
      "mid",
      "senior",
    ...
    photo: asStr(raw.photo).startsWith("data:image/")
      ? asStr(raw.photo)
      : undefined,
  };
...
export function loadResume(): Resume | null {
  try {
    ...
    if (!Array.isArray((parsed as Record<string, unknown>).experience))
      return null;
...
export function listResumeVersions(): ResumeVersion[] {
  try {
    ...
    return parsed.flatMap((v) => {
      ...
      if (!data) return [];
      const folder =
        typeof v.folder === "string" && v.folder.trim() ? v.folder : undefined;
...
function persistHistory(snapshots: ResumeSnapshot[]) {
  try {
    localStorage.setItem(
      HISTORY_KEY,
      JSON.stringify(snapshots.slice(0, HISTORY_MAX)),
    );
  } catch {
    // storage full / private mode — ignore
  }
 ...
 */
export function recordResumeSnapshot(
  data: Resume,
  force = false,
): ResumeSnapshot[] {
  ...
  const next = [
    { id: newId(), at: Date.now(), data: JSON.parse(json) as Resume },
    ...history,
  ];
  persistHistory(next);
...
export function listExperienceLibrary(): SavedExperience[] {
  try {
    ...
    if (!Array.isArray(parsed)) return [];
    return parsed.flatMap((s) => {
      if (!s || typeof s !== "object" || !s.id || typeof s.savedAt !== "number")
        return [];
...
export function saveExperienceToLibrary(
...
function sanitizeEducationItem(input: unknown): EducationItem | null {
  ...
  return item.school.trim() || item.degree.trim() || item.details.trim()
    ? item
    : null;
}
...
function persistEducationLibrary(items: SavedEducation[]) {
  try {
    localStorage.setItem(
      ...
      JSON.stringify(items.slice(0, EDUCATION_LIBRARY_MAX)),
    );
  } catch {
    // storage full / private mode — ignore
  }
...
function sanitizeProjectItem(input: unknown): ProjectItem | null {
  ...
  return item.name.trim() || item.link.trim() || item.description.trim()
    ? item
    : null;
}
...
function persistProjectLibrary(items: SavedProject[]) {
  try {
    localStorage.setItem(
      PROJECT_LIBRARY_KEY,
      JSON.stringify(items.slice(0, PROJECT_LIBRARY_MAX)),
    );
  } catch {
    // storage full / private mode — ignore
  }
...
function sanitizeInvolvementItem(input: unknown): InvolvementItem | null {
  ...
  return item.role.trim() || item.organization.trim() || item.description.trim()
    ? item
    : null;
}
...
export function saveInvolvementToLibrary(
  entry: InvolvementItem,
): SavedInvolvement[] {
...
function sanitizeCourseworkItem(input: unknown): CourseworkItem | null {
  ...
  return item.name.trim() || item.institution.trim() || item.description.trim()
    ? item
    : null;
}
...
export function saveCourseworkToLibrary(
  entry: CourseworkItem,
): SavedCoursework[] {
...
function sanitizeAwardItem(input: unknown): AwardItem | null {
  ...
  return item.name.trim() || item.organization.trim() || item.description.trim()
    ? item
    : null;
}
...
function persistAwardLibrary(items: SavedAward[]) {
  try {
    localStorage.setItem(
      AWARD_LIBRARY_KEY,
      JSON.stringify(items.slice(0, AWARD_LIBRARY_MAX)),
    );
  } catch {
    // storage full / private mode — ignore
  }
...
function sanitizeReferenceItem(input: unknown): ReferenceItem | null {
  ...
  return item.name.trim() || item.employer.trim() || item.email.trim()
    ? item
    : null;
}
...
function sanitizeCertificationItem(input: unknown): CertificationItem | null {
  ...
  return item.name.trim() || item.issuer.trim() || item.description.trim()
    ? item
    : null;
}
...
function persistCertLibrary(items: SavedCertification[]) {
  try {
    localStorage.setItem(
      CERT_LIBRARY_KEY,
      JSON.stringify(items.slice(0, CERT_LIBRARY_MAX)),
    );
  } catch {
    // storage full / private mode — ignore
  }
...
export function saveCertToLibrary(
...
export function savePublicationToLibrary(
...
function persistSkillsLibrary(items: SavedSkills[]) {
  try {
    localStorage.setItem(
      SKILLS_LIBRARY_KEY,
      JSON.stringify(items.slice(0, SKILLS_LIBRARY_MAX)),
    );
  } catch {
    // storage full / private mode — ignore
  }
...
function persistSummaryLibrary(items: SavedSummary[]) {
  try {
    localStorage.setItem(
      SUMMARY_LIBRARY_KEY,
      JSON.stringify(items.slice(0, SUMMARY_LIBRARY_MAX)),
    );
  } catch {
    // storage full / private mode — ignore
  }
...
export function involvementHeadingLine(i: InvolvementItem): string {
  const left = [i.role.trim(), i.organization.trim()]
    .filter(Boolean)
    .join("  ·  ");
...
export function publicationHeadingLine(p: PublicationItem): string {
  const head = [p.title.trim(), p.venue.trim()].filter(Boolean).join(" — ");
...
export function resumeToPlainText(r: Resume): string {
  ...
  lines.push(
    [c.email, c.phone, c.location, c.website, c.linkedin]
      .filter(Boolean)
      .join(" | "),
  );
  for (const key of orderedSectionKeys(r)) {
    if (key === "summary" && r.summary) {
      ...
      key === "experience" &&
      r.experience.some((e) => e.company || e.role)
    ) {
      ...
      for (const cw of courseworkEntries(r)) {
        lines.push(
          courseworkHeadingLine(cw) +
            (cw.date.trim() ? ` (${cw.date.trim()})` : ""),
        );
      ...
      key === "certifications" &&
      (certEntries(r).length > 0 || r.certifications)
    ) {
      ...
      for (const p of publicationEntries(r)) {
        lines.push(
          publicationHeadingLine(p) +
            (p.date.trim() ? ` (${p.date.trim()})` : ""),
        );
      ...
      for (const a of agentEntries(r)) {
        lines.push(
          a.name.trim() + (a.date.trim() ? ` (${a.date.trim()})` : ""),
        );
...
export function resumeToMarkdown(r: Resume): string {
  ...
  const c = r.contact;
  lines.push(`# ${[c.fullName, c.title].filter(Boolean).join(" — ")}`);
  const contact = [c.email, c.phone, c.location, c.website, c.linkedin].filter(
    Boolean,
  );
  ...
  for (const key of orderedSectionKeys(r)) {
    if (key === "summary" && r.summary) {
      ...
      for (const e of r.experience) {
        ...
        lines.push(
          `### ${[e.role, e.company].filter(Boolean).join(" — ")}${dates}`,
          "",
        );
        for (const b of e.bullets) if (b.trim()) lines.push(`- ${b.trim()}`);
        lines.push("");
      ...
      for (const i of involvementEntries(r)) {
        const dates = involvementDates(i);
        lines.push(
          `### ${involvementHeadingLine(i)}${dates ? ` *(${dates})*` : ""}`,
          "",
        );
        for (const b of involvementBullets(i)) lines.push(`- ${b}`);
        lines.push("");
      ...
      for (const e of r.education) {
        ...
        const dates =
          e.startDate || e.endDate ? ` *(${e.startDate} – ${e.endDate})*` : "";
        lines.push(
          `### ${[e.degree, e.school].filter(Boolean).join(", ")}${dates}`,
          "",
        );
        const detail = educationDetailLine(e);
        if (detail) lines.push(detail, "");
      ...
      for (const m of militaryEntries(r)) {
        const dates = militaryDates(m);
        lines.push(
          `### ${militaryHeadingLine(m)}${dates ? ` *(${dates})*` : ""}`,
          "",
        );
        for (const b of militaryBullets(m)) lines.push(`- ${b}`);
        lines.push("");
