...
export async function downloadResumeDocx(resume: Resume, filename: string) {
  ...
  const bulletInd = bulletIndentOf(resume)
    ? { left: 920, hanging: 360 }
    : undefined;
    ...
    new Paragraph({
      ...
      shading: tpl.band
        ? {
            ...
            fill: accentTint(tpl.accent).replace("#", ""),
          }
        : undefined,
      border:
  ...
  for (const key of orderedSectionKeys(resume)) {
    if (key === "summary" && resume.summary.trim()) {
      children.push(
        heading("Summary"),
        body(resume.summary.trim(), { after: 100 }),
      );
    } else if (
      key === "experience" &&
      ...
      for (const p of resume.projects) {
        ...
        if (p.description.trim())
          children.push(body(p.description.trim(), { after: 80 }));
      }
      ...
      for (const i of involvementEntries(resume)) {
        ...
        for (const b of involvementBullets(i))
          children.push(body(b, { bullet: true }));
      }
      ...
      for (const cw of courseworkEntries(resume)) {
        ...
        for (const b of courseworkBullets(cw))
          children.push(body(b, { bullet: true }));
      }
      ...
      for (const c of certEntries(resume)) {
        ...
        if (c.description.trim())
          children.push(body(c.description.trim(), { after: 80 }));
      }
      ...
      for (const a of awardEntries(resume)) {
        ...
        for (const b of awardBullets(a))
          children.push(body(b, { bullet: true }));
      }
    } else if (
      key === "publications" &&
      publicationEntries(resume).length > 0
    ) {
      ...
      for (const p of publicationEntries(resume)) {
        ...
        for (const b of publicationBullets(p))
          children.push(body(b, { bullet: true }));
      }
      ...
      for (const x of referenceEntries(resume)) {
        const role = [x.title.trim(), x.employer.trim()]
          .filter(Boolean)
          .join(", ");
        children.push(
          new Paragraph({
      ...
      for (const m of militaryEntries(resume)) {
        ...
        for (const b of militaryBullets(m))
          children.push(body(b, { bullet: true }));
      }
  ...
  if (c.fullName.trim()) {
    paragraphs.push(
      new Paragraph({
        spacing: { after: 40 },
        children: [
          new TextRun({ text: c.fullName.trim(), bold: true, size: 30, font }),
        ],
      }),
    );
  }
